...
- **Interactive Video Scrubber**: Full-width edge-to-edge progress timeline with iPhone Photos/Reels-style scrubber. Features 3px orange track (#FF8C00), draggable thumb for precise seeking with sub-second accuracy, dual-path support (VideoPlayer + HtmlElementView), and automatic playback resumption after seeking to maintain audio sync. Uses GlobalKey dynamic positioning to measure overlay height and position scrubber 8px above overlay content, adapting to varying caption lengths and screen sizes.
