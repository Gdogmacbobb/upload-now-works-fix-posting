...
- **Unified Thumbnail-Preview Button**: Interactive preview element that merges thumbnail display and video preview into a single tappable component. Features 85% width centered container with fixed 4:5 (0.8) aspect ratio, displaying the current video frame (paused at selected timestamp) with centered play icon overlay (56px, semi-transparent) and duration badge. Automatically shows first frame (Duration.zero) as default thumbnail on upload screen load using silent play-pause technique: mute controller, play 200ms to force texture rendering (critical for web), pause at frame, restore volume. Uses PointerInterceptor for web compatibility to capture tap events above HtmlElementView platform views. Tapping opens full-screen TikTok-style preview with synchronized audio replay. 
- **TikTok-Style Thumbnail Selector**: Compact single-frame thumbnail selector with fixed 4:5 (0.8) aspect ratio matching TikTok's upload flow proportions. Features semi-transparent white frame border overlay to show thumbnail boundary, dedicated scrub bar for precise frame navigation, and compact floating checkmark confirmation button (30x30). Automatically detects video orientation and applies 90° clockwise rotation for landscape videos to display them upright in portrait mode. Stores selected frame timestamp (in milliseconds) and updates unified preview by seeking main controller. Uses separate VideoPlayerController for thumbnail preview to prevent interference with main video playback. Database field `thumbnail_frame_time` added to `videos` table for persistence.
