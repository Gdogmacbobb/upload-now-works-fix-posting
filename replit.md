...
- **Web Transform Cleanup**: Full-screen video preview implements platform-specific transform cleanup on dispose. For Replit sandbox (HtmlElementView), removes transform/will-change/object-fit properties. For standard Flutter web, stores original transforms on dialog open and restores them on close—if original transform was empty, removes the property to clear rotation; if original had value, restores that value to preserve layout. Prevents rotation persistence when returning to upload screen.
