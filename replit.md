...
- **Fixed double SafeArea issue**: Removed outer SafeArea wrapper around FeedNavigationHeaderWidget in both feeds, positioning the header directly at `top: 0` using Positioned widget. The gradient background now extends from screen edge behind the notch, while inner SafeArea protects text from overlap. This eliminates the black gap that appeared above the navigation bar when SafeArea padding was applied twice
