...
class AppRoutes {
  ...
  // Safe navigation helper with role-based access control
  static Future<T?> pushNamed<T extends Object?>(
    ...
    try {
      // Check role-based access for performer-only routes
      if (_isPerformerOnlyRoute(routeName)) {
        final roleService = RoleService.instance;
        if (!roleService.hasPermission(_getRequiredFeature(routeName))) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('This feature is available for Street Performers only. Upgrade your account to access performer tools.'),
  ...
  // Check if route requires performer permissions
  static bool _isPerformerOnlyRoute(String routeName) {
    return [
      videoRecording,
      videoUpload,
    ].contains(routeName);
  ...
  // Get required feature for performer-only routes
  static PerformerFeature _getRequiredFeature(String routeName) {
    switch (routeName) {
      case videoRecording:
        return PerformerFeature.recordVideo;
      case videoUpload:
        return PerformerFeature.uploadVideo;
      default:
        return PerformerFeature.browseFeeds; // Fallback for shared features
    }
  }
